...
def add_options():
  ...
  flags.DEFINE_string('input_dir', default = '/root/af_input', help = 'path to input directory')
  flags.DEFINE_string('output_dir', default = '/root/af_output', help = 'path to output directory')
  flags.DEFINE_string('service_host', default = '0.0.0.0', help = 'service host')
  flags.DEFINE_integer('service_port', default = 8083, help = 'service port')
  flags.DEFINE_string('public_databases', default = '/root/public_databases', help = 'path to public databases')
  flags.DEFINE_string('model_dir', default = '/root/models', help = 'path to pretrained model')

class AlphaFold3Manager(object):
  ...
  def run_alphafold3(self, gpu_id, input_file, seed = 1):
    ...
    try:
      ...
      af3 = AF3(FLAGS.model_dir, FLAGS.input_dir, FLAGS.output_dir, public_databases = FLAGS.public_databases)
      with self.lock:
        ...
        self.logs[gpu_id].append(f"{datetime.now()}: start new AlphaFold3 task")
      return True, f"started task on GPU {gpu_id}, output directory: {join(FLAGS.output_dir, str(gpu_id))}"
    except Exception as e:
      return False, f"failed to start AlphaFold3: {str(e)}"
...
def main(unused_argv):
  ...
  global application
  import uvicorn
  manager = AlphaFold3Manager(FLAGS.num_gpus)
